...
class RecurringEvent(EventFields):
    ...
    end_date = models.DateField(help_text="Date of the last occurance of this event",validators=[MaxValueValidator(limit_value=getMaxRecurringTime)])
...
class Event(EventFields):
    ...
    def add_hosts(self, *clubs: list[Club], commit=True):
        """Add list of hosts to event."""

        for club in clubs:
            self.add_host(club, commit=False)
