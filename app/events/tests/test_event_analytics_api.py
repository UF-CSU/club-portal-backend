...
class EventAnalyticsTestsBase(PrivateApiTestsBase):
    ...
    def assertEventAnalytics(self, res: Response, data: EventAnalytics):
        """Check the analytics data in an event response."""

        analytics = res.json()["analytics"]

        # Verify event analytics
        self.assertEqual(analytics["users_total"], data.event_users_total)
        self.assertEqual(analytics["guests_total"], data.event_members_total)
        self.assertEqual(analytics["returning_total"], data.event_returning_total)
        ...
        self.assertEqual(
            analytics["previous_event"]["users_diff"], data.prev_users_diff
        )
        self.assertEqual(
            analytics["previous_event"]["members_total"], data.prev_members_total
        )
        self.assertEqual(
            analytics["previous_event"]["members_diff"], data.prev_members_diff
        )
        self.assertEqual(
            analytics["previous_event"]["returning_total"], data.prev_returning_total
        )
        self.assertEqual(
            analytics["previous_event"]["returning_diff"], data.prev_returning_diff
        )
        ...
        self.assertEqual(analytics["event_type"]["users_avg"], data.evtype_users_avg)
        self.assertEqual(analytics["event_type"]["users_diff"], data.evtype_users_diff)
        self.assertEqual(
            analytics["event_type"]["members_avg"], data.evtype_members_avg
        )
        self.assertEqual(
            analytics["event_type"]["members_diff"], data.evtype_members_diff
        )
        self.assertEqual(
            analytics["event_type"]["returning_avg"], data.evtype_returning_avg
        )
        self.assertEqual(
            analytics["event_type"]["returning_diff"], data.evtype_returning_diff
        )
        ...
        self.assertEqual(analytics["recurring_event"]["users_avg"], data.rec_users_avg)
        self.assertEqual(
            analytics["recurring_event"]["users_diff"], data.rec_users_diff
        )
        self.assertEqual(
            analytics["recurring_event"]["members_avg"], data.rec_members_avg
        )
        self.assertEqual(
            analytics["recurring_event"]["members_diff"], data.rec_members_diff
        )
        self.assertEqual(
            analytics["recurring_event"]["returning_avg"], data.rec_returning_avg
        )
        self.assertEqual(
            analytics["recurring_event"]["returning_diff"], data.rec_returning_diff
        )
...
class EventBasicAnalyticsApiTests(EventAnalyticsTestsBase):
    ...
    def test_default_analytics_in_details(self):
        ...
        self.assertEventAnalytics(res, EventAnalytics(event.pk))
...
class EventAnalyticsApiCalculationTests(EventAnalyticsTestsBase):
    ...
    def setUp(self):
        ...
        CSU GBMs:
        - 1/12: 80 users, 18 members, 0 returning
        - 2/17: 50 users, 15 members, 40 returning

        CSU Dev Meetings:
        - [0] 1/12: 15 users, 15 members, 0 returning
        ...
        CSU Exec Meetings:
        - [0] 1/16: 5 users, 5 members, 0 returning
        ...
        OSC GBMs:
        - 1/14: 80 users, 8 members, 0 returning
        ...
        if self.LOAD_CACHED_DATA:
            ...
            self.csu_dev_meetings = csu_dev_meetings_rec.events.all()
            csu_exec_meetings_rec = RecurringEvent.objects.get(
                name="CSU Weekly Exec Meeting"
            )
            self.csu_exec_meetings = csu_exec_meetings_rec.events.all()
            ...
            self.csu_gbm_1 = create_test_event(
                ...
                start_at=datetime(2026, 1, 13, 18, 0, 0),
                end_at=datetime(2026, 1, 13, 20, 0, 0),
            ...
            self.csu_dev_meetings = RecurringEventService(
                csu_dev_meetings_rec
            ).sync_events()
            ...
            self.csu_exec_meetings = RecurringEventService(
                csu_exec_meetings_rec
            ).sync_events()
            ...
            self.osc_gbm_1 = create_test_event(
                ...
                name="OSC GBM 1",
                event_type=EventType.GBM,
                start_at=datetime(2026, 1, 14, 18, 0, 0),
                end_at=datetime(2026, 1, 14, 20, 0, 0),
                enable_attendance=True,
            )
            ...
            self.osc_ccs = RecurringEventService(osc_ccs_rec).sync_events()
            ...
            self.osc_exec_meetings = RecurringEventService(
                osc_exec_meetings_rec
            ).sync_events()
            ...
            # Each meeting
            for i, user in enumerate(users[80:88]):
                csu_svc.add_member(user)

                if i < 12:  # Events with 12+ members
                ...
                elif i < 13:  # Events with 13+ members
                    create_test_attendance(self.csu_dev_meetings[0], user)
                    create_test_attendance(self.csu_dev_meetings[1], user)
                    create_test_attendance(self.csu_dev_meetings[2], user)
                    create_test_attendance(self.csu_dev_meetings[3], user)
                    ...
                    create_test_attendance(self.csu_dev_meetings[7], user)
                    create_test_attendance(self.csu_dev_meetings[9], user)
                elif i < 14:  # Events with 14+ members
                    create_test_attendance(self.csu_dev_meetings[0], user)
                    create_test_attendance(self.csu_dev_meetings[1], user)
                    create_test_attendance(self.csu_dev_meetings[2], user)
                    create_test_attendance(self.csu_dev_meetings[3], user)
                    ...
                    create_test_attendance(self.csu_dev_meetings[5], user)
                    create_test_attendance(self.csu_dev_meetings[6], user)
                elif i < 15:  # Events with 15+ members
                    create_test_attendance(self.csu_dev_meetings[0], user)
                    create_test_attendance(self.csu_dev_meetings[1], user)
                    create_test_attendance(self.csu_dev_meetings[2], user)
                    create_test_attendance(self.csu_dev_meetings[3], user)
                    create_test_attendance(self.csu_dev_meetings[4], user)
                    create_test_attendance(self.csu_dev_meetings[5], user)
                elif i < 16:  # Events with 16+ members
                    create_test_attendance(self.csu_dev_meetings[1], user)
                    create_test_attendance(self.csu_dev_meetings[2], user)
                    create_test_attendance(self.csu_dev_meetings[4], user)
                elif i < 18:  # Events with 18+ members
                    create_test_attendance(self.csu_dev_meetings[1], user)
                    create_test_attendance(self.csu_dev_meetings[2], user)
                elif i < 20:  # Events with 20+ members
                    create_test_attendance(self.csu_dev_meetings[1], user)
    ...
    def test_event_average_analytics(self):
        ...
        expected_analytics = EventAnalytics(
            ...
            prev_returning_diff=3,
            # Event type
            evtype=event.event_type,
            evtype_events_count=self.csu_dev_meetings.count()
            + self.csu_exec_meetings.count(),
            evtype_users_avg=11.5,
            evtype_users_diff=18 - 11.5,
            evtype_members_avg=11.5,
            evtype_members_diff=18 - 11.5,
            evtype_returning_avg=5,
            evtype_returning_diff=18 - 5,
            # Recurring event
            rec_events_count=2,
            rec_users_avg=17.5,
            rec_users_diff=18 - 17.5,
            rec_members_avg=17.5,
            rec_members_diff=18 - 17.5,
            rec_returning_avg=7.5,
            rec_returning_diff=18,
        )
